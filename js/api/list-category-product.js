...
function listUser() {
	...
	const login = 'https://api.duaca.top/api/admin/list-category-product';
	fetch(login, {
		method: "get",
		headers: {
		...
		.then((data) => {
			...
			data.payload.forEach(d => {
				html += "<tr>";
				html += "<td>" + d.id + "</td>";
				html += "<td>" + d.category_products_name + "</td> ";
				html += "<td>" + d.category_product_code+ "</td> ";
				html += "<td style='text-align:center;'>"
					+ "<a href='javascript:void(0)' class='btn btn-warning edit_user' id='edit_user' data-id='" + d.id + "' data-category_products_name='" + d.category_products_name + "' data-category_product_code='" + d.category_product_code  + "'>Cập nhật</a>"
					+ "<a href='javascript:void(0)' class='btn btn-danger delete_user' data-id='" + d.id + "' data-category_product_code='" + d.category_product_code + "' >Xóa</a>"
					+ "</td> ";
				html += "</tr>";
			});
			...
			for (var i = 0; i < delete_user.length; i++) {
				delete_user[i].addEventListener('click', function () {
					...
					Swal.fire({
						...
						if (result.value) {
							fetch("https://api.duaca.top/api/admin/remove-category-product", {
								method: "POST",
								headers: {
									Accept: "application/json, text/plain, */*",
									"Content-Type": "application/json",
								...
								body: JSON.stringify({
									id : this.dataset.id ,
									category_product_code : this.dataset.category_product_code ,
								}),
							})
								.then((response) => response.json())
								.then((data) => {
					...
					Swal.fire({
						...
						html: `
							<table>
								...
								<tr>
									<td class="td1">category_products_name : </td>
									<td class="td2"><input  class="form-control" id="name" value="` + this.dataset.category_products_name + `"></td>
								</tr>
								<tr>
									<td class="td1">category_product_code : </td>
									<td class="td2"><input  class="form-control" id="code" value="` + this.dataset.category_product_code + `"></td>
								</tr>
							 </table>
							 `,
						preConfirm: function() {
					...
					.then((result) => {
					    ...
					    if (result.isConfirmed) {
				            fetch("https://api.duaca.top/api/admin/save-category-product", {
							        ...
							        body: JSON.stringify({
							        	category_product_code: document.getElementById('code').value,
				                ...
				                .then((data) => {
				                    console.log(data);
				                    if(data.message==""){
				                    	alertDone('Thông Báo','Đã sửa danh mục sản phẩm thành công , nhấn vào refresh hoặc tải lại trang để làm mới dữ liệu');
				                    }else{
